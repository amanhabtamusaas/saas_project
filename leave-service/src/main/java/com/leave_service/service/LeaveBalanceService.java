...
public class LeaveBalanceService {
    ...
    @Transactional
    public LeaveBalanceResponse calculateLeaveBalance(UUID tenantId, UUID employeeId) {
        ...
        try {
            ...
            saveCurrentLeaveBalanceToHistory(employeeId, totalAvailableLeaveDays,
                    currentRemainingLeaveDays, currentBudgetYear.getId(), tenantId);
            ...
            logger.error("Error calculating leave balance for employee ID: {} in tenant ID: {}",
                    employeeId, tenantId, e);
            throw e;
        }
    }

    @Transactional
    public void saveCurrentLeaveBalanceToHistory(UUID employeeId, int totalLeaveDays, int remainingLeaveDays,
                                                 UUID budgetYearId, UUID tenantId) {
        ...
        try {
            ...
            logger.info("Successfully saved leave balance history for employee ID: {} in tenant ID: {}",
                    employeeId, tenantId);
        } catch (Exception e) {
            // Log the error with appropriate details
            logger.error("Error saving current leave balance to history for employee ID: {} in tenant ID: {}",
                    employeeId, tenantId, e);
    ...
    public List<LeaveBalanceHistory> getLeaveBalanceHistory(UUID employeeId, UUID tenantId) {
        return leaveBalanceHistoryRepository
                .findByEmployeeIdAndTenantIdOrderByCalculationDateDesc(employeeId, tenantId);
    }
